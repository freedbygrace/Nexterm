...
void guac_spice_session_configure(guac_client* client, SpiceSession* session) {
    ...
    if (settings->tls && settings->tls_port != NULL) {
        ...
        /* Provide CA certificate for verification, if given. The value may be
         * either the path of a PEM file or the PEM data itself, as brokers like
         * the Proxmox VE spiceproxy API return the cluster CA inline with the
         * connection ticket and have no file to point at. */
        if (settings->ca_file != NULL) {

            if (strstr(settings->ca_file, "-----BEGIN") != NULL) {
                GByteArray* ca = g_byte_array_new();
                g_byte_array_append(ca, (const guint8*) settings->ca_file,
                        strlen(settings->ca_file) + 1);
                g_object_set(session, "ca", ca, NULL);
