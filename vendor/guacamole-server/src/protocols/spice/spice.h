...
/**
 * State for a single SPICE display channel. A SPICE guest may expose more than
 * one display channel: a Linux guest with a single multi-head QXL device
 * presents all heads as regions of one combined framebuffer (a single display
 * channel), but a Windows guest presents each head as a separate QXL device,
 * and therefore a separate display channel, each with its own primary surface.
 * guacd composites the surfaces of all active channels side by side into the
 * single Guacamole default layer, then publishes a multi-monitor layout
 * describing where each head lands within that combined layer.
 */
typedef struct guac_spice_display_state {

    /**
     * The SPICE display channel this slot tracks, or NULL if unused. Slots are
     * indexed by SPICE channel id.
     */
    SpiceChannel* channel;

    /**
     * Pointer to this channel's primary surface buffer, or NULL if no primary
     * surface currently exists for the channel.
     */
    void* data;

    /**
     * The width and height (in pixels) and stride (in bytes per row) of this
     * channel's primary surface.
     */
    int width;
    int height;
    int stride;
     ...
     * this channel's primary surface.
     */
    int format;

    /**
     * The position of the top-left corner of this channel's surface within the
     * combined default layer, in pixels. Active channels are tiled left to
     * right in channel-id order.
     */
    int origin_x;
    int origin_y;

} guac_spice_display_state;
...
typedef struct guac_spice_client {
    ...
    /**
     * Lock which synchronizes access to the per-display-channel surface
     * metadata below, which is updated from SPICE display channel signal
     * handlers and read while compositing damaged regions into the
     * guac_display.
     */
    pthread_mutex_t surface_lock;

    /**
     * Per-display-channel surface state, indexed by SPICE channel id. A SPICE
     * guest may expose multiple display channels (one per QXL device / head);
     * the surfaces of all active channels are composited side by side into the
     * single Guacamole default layer. Guarded by surface_lock.
     */
    guac_spice_display_state displays[GUAC_SPICE_MAX_MONITORS];

    /**
     * The dimensions of the current combined default layer (the bounding box
     * of all composited display-channel surfaces), in pixels. Guarded by
     * surface_lock.
     */
    int combined_width;
    int combined_height;
