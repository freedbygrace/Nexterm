 ...
 *
 * @param spice_client
 *     The SPICE client whose session recording should be annotated.
 *
 * @param outbound
 ...
 *
 * @param stream_index
 *     The index of the Guacamole stream carrying the clipboard data, allowing
 *     the annotation to be correlated with the surrounding instructions, or a
 *     negative value if the transfer is not carried by a stream.
 *
 * @param mimetype
 ...
 */
static void guac_spice_clipboard_record_direction(
        guac_spice_client* spice_client, int outbound, int stream_index,
        const char* mimetype, int length) {

    guac_recording* recording = spice_client->recording;

    /* Annotate only if a recording is in progress and clipboard recording was
     * explicitly requested */
    if (recording == NULL || recording->socket == NULL
            || !spice_client->settings->recording_include_clipboard)
        return;
    ...
    if (stream_index < 0)
        guac_protocol_send_log(recording->socket,
                "clipboard direction=%s mimetype=%s bytes=%d",
                direction, mimetype, length);
    else if (length >= 0)
        guac_protocol_send_log(recording->socket,
...
static void guac_spice_clipboard_selection(SpiceMainChannel* channel,
    ...
    guac_spice_clipboard_record_direction(spice_client, 1, -1, mimetype,
            (int) size);
...
int guac_spice_clipboard_handler(guac_user* user, guac_stream* stream,
    ...
    guac_spice_clipboard_record_direction(spice_client, 0, stream->index,
            mimetype, -1);
