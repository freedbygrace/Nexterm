...
int guac_spice_user_mouse_handler(guac_user* user, int x, int y, int mask) {
    ...
    /* Map the combined-framebuffer coordinate onto the display channel that owns
     * it, and to that display's local coordinates. A multi-head guest may
     * expose each head as a separate display channel composited side by side;
     * SPICE positions the pointer per display, so it must be routed to the
     * correct head using head-local coordinates. For a single combined surface
     * this resolves to display 0 with the coordinate unchanged. */
    int display_id = 0;
    int local_x = x;
    int local_y = y;
    pthread_mutex_lock(&spice_client->surface_lock);
    /* Clamp the combined coordinate into the composited bounds so a value on the
     * far right/bottom edge maps to the last display rather than falling through
     * to display 0. */
    int cx = x, cy = y;
    if (spice_client->combined_width > 0 && cx >= spice_client->combined_width)
        cx = spice_client->combined_width - 1;
    if (spice_client->combined_height > 0 && cy >= spice_client->combined_height)
        cy = spice_client->combined_height - 1;
    if (cx < 0) cx = 0;
    if (cy < 0) cy = 0;
    for (int i = 0; i < GUAC_SPICE_MAX_MONITORS; i++) {
        ...
        if (d->data == NULL || d->width <= 0 || d->height <= 0)
            continue;
        if (cx >= d->origin_x && cx < d->origin_x + d->width
                && cy >= d->origin_y && cy < d->origin_y + d->height) {
            display_id = i;
            local_x = cx - d->origin_x;
            local_y = cy - d->origin_y;
            break;
        }
    }
    ...
    if (spice_client->mouse_mode == SPICE_MOUSE_MODE_SERVER)
        ...
        guac_spice_defer_position(inputs, local_x, local_y, display_id, spice_mask);
